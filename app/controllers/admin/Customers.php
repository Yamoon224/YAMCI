...
class Customers extends MY_Controller
{
    ...
    public function index($action = null)
    {
        ...
        // Stats KPI Clients
        $stats = new stdClass();
        $stats->total = 0; $stats->active = 0; $stats->with_deposit = 0; $stats->total_deposit = 0;
        try {
            $q = $this->db->query("SELECT
                COUNT(*) AS total,
                SUM(CASE WHEN award_points >= 0 THEN 1 ELSE 0 END) AS active,
                SUM(CASE WHEN deposit_amount > 0 THEN 1 ELSE 0 END) AS with_deposit,
            ...
            if ($q && $q->num_rows() > 0) {
                ...
                $stats->with_deposit  = (int)   $r->with_deposit;
                $stats->total_deposit = (float) $r->total_deposit;
            }
        } catch (\Throwable $e) { /* silencieux */ }
        $this->data['stats'] = $stats;

        // Listes pour selects de filtre
        try {
            $q = $this->db->query("SELECT DISTINCT customer_group_name FROM " . $this->db->dbprefix('companies') . " WHERE group_name='customer' AND customer_group_name IS NOT NULL AND customer_group_name != '' ORDER BY customer_group_name");
            $this->data['filter_customer_groups'] = $q ? $q->result() : [];
        } catch (\Throwable $e) { $this->data['filter_customer_groups'] = []; }
        try {
            $q = $this->db->query("SELECT DISTINCT price_group_name FROM " . $this->db->dbprefix('companies') . " WHERE group_name='customer' AND price_group_name IS NOT NULL AND price_group_name != '' ORDER BY price_group_name");
            $this->data['filter_price_groups'] = $q ? $q->result() : [];
        } catch (\Throwable $e) { $this->data['filter_price_groups'] = []; }
