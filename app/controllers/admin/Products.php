...
class Products extends MY_Controller
{
    ...
    public function add($id = null)
    {
        ...
        if (true == $this->form_validation->run()) {
            ...
            if ('standard' == $this->input->post('type')) {
                    ...
                    for ($r = 0; $r <= $a; $r++) {
                        if (isset($_POST['attr_name'][$r])) {
                            $product_attributes[] = [
                                ...
                                'quantity'     => $_POST['attr_quantity'][$r] ?? 0,
                ...
                if ($wh_total_quantity != $pv_total_quantity && 0 != $pv_total_quantity) {
            ...
            if ('service' == $this->input->post('type')) {
            ...
            } elseif ('combo' == $this->input->post('type')) {
                $total_price = 0;
                ...
                $data['track_quantity'] = 0;
            } elseif ('digital' == $this->input->post('type')) {
        ...
        if (true == $this->form_validation->run() && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            ...
            $this->data['combo_items'] = ($id && 'combo' == $this->data['product']->type) ? $this->products_model->getProductComboItems($id) : null;
            ...
            if ($_FILES['csv_file']['size'] > 0) {
                ...
                foreach ($final as $pr) {
                    if ($product = $this->products_model->getProductByCode(trim($pr['code']))) {
                        ...
                        if (!$this->Settings->overselling && 'subtraction' == $type) {
                            if ($variant) {
                                if ($op_wh_qty = $this->products_model->getProductWarehouseOptionQty($variant, $warehouse_id)) {
                                    if ($op_wh_qty->quantity < $quantity) {
        ...
        if (true == $this->form_validation->run()) {
            if (!empty($_POST['val'])) {
                if ('delete' == $this->input->post('form_action')) {
                    ...
                    redirect($_SERVER['HTTP_REFERER']);
                } elseif ('export_excel' == $this->input->post('form_action')) {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        if ($items) {
                            foreach ($items as $item) {
                                $products .= $item->product_name . '(' . $this->sma->formatQuantity('subtraction' == $item->type ? -$item->quantity : $item->quantity) . ')' . "\n";
                            }
                        }
    ...
    public function count_stock($page = null)
    {
        ...
        if (true == $this->form_validation->run() && $this->products_model->addStockCount($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if (true == $this->form_validation->run() && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)) {
            ...
            $this->data['combo_items'] = 'combo' == $product->type ? $this->products_model->getProductComboItems($product->id) : null;
            $this->data['product_options'] = $id ? $this->products_model->getProductOptionsWithWH($id) : null;
    ...
    public function edit_adjustment($id)
    {
        ...
        if (true == $this->form_validation->run() && $this->products_model->updateAdjustment($id, $data, $products)) {
            $this->session->set_userdata('remove_qals', 1);
            $this->session->set_flashdata('message', lang('quantity_adjusted'));
            admin_redirect('products/quantity_adjustments');
        } else {
    ...
    public function finalize_count($id)
    {
        ...
        if (true == $this->form_validation->run() && $this->products_model->finalizeStockCount($id, $data, $products)) {
    ...
    public function import_csv()
    {
            ...
            if (isset($_FILES['userfile'])) {
                ...
                foreach ($arrResult as $key => $value) {
                    ...
                    $item = [
                        ...
                        'tax_method'        => isset($value[12]) ? ('exclusive' == trim($value[12]) ? 1 : 0) : '',
        ...
        if (true == $this->form_validation->run() && !empty($items)) {
    ...
    public function index($warehouse_id = null)
    {
        ...
        /* ────────────────────────────────────────────────────────────────
         * Stats widget (style Pixinvent Materialize : 4 KPIs)
         * ──────────────────────────────────────────────────────────────── */
        $stats = new stdClass();
        try {
        ...
        $stats->stock_by_price = 0;
        $stats->stock_by_cost  = 0;
        try {
            $this->load->admin_model('reports_model');
            if (method_exists($this->reports_model, 'getStockValue')) {
                $stockValue = $this->reports_model->getStockValue();
                if ($stockValue) {
                    $stats->stock_by_price = (float) ($stockValue->stock_by_price ?? 0);
                    $stats->stock_by_cost  = (float) ($stockValue->stock_by_cost  ?? 0);
                }
            }
        ...
        $stats->low_stock = 0;
        try {
            $whClause = $warehouse_id ? 'WHERE wp.warehouse_id = ' . (int) $warehouse_id : '';
            $sql = 'SELECT COUNT(*) as nb FROM (
                SELECT p.id, COALESCE(SUM(wp.quantity),0) as qty, p.alert_quantity
                FROM ' . $this->db->dbprefix('products') . ' p
                LEFT JOIN ' . $this->db->dbprefix('warehouses_products') . ' wp ON wp.product_id = p.id
                ' . $whClause . '
                GROUP BY p.id, p.alert_quantity
                HAVING p.alert_quantity > 0 AND qty <= p.alert_quantity
            ) a';
            $q = $this->db->query($sql);
            if ($q && $q->num_rows() > 0) {
                $stats->low_stock = (int) $q->row()->nb;
            }
        } catch (\Throwable $e) { /* silencieux */ }

        $this->data['stats'] = $stats;

        // Catégories et marques pour les selects de filtre (header)
        $this->data['categories'] = $this->site->getAllCategories();
        $this->data['brands']     = $this->site->getAllBrands();
    ...
    public function print_barcodes($product_id = null)
    {
        ...
        if (true == $this->form_validation->run()) {
            ...
            $bci_size = (10 == $style || 12 == $style ? 50 : (14 == $style || 18 == $style ? 30 : 20));
                    ...
                    foreach ($variants as $option) {
                        if ($this->input->post('vt_' . $product->id . '_' . $option->id)) {
                            $barcodes[] = [
                                ...
                                'price'      => $this->input->post('price') ? $this->sma->formatMoney(0 != $option->price ? ($product->price + $option->price) : $product->price, 'none') : false,
                                'rprice'     => $this->input->post('price') ? (0 != $option->price ? ($product->price + $option->price) : $product->price) : false,
        ...
        if (true == $this->form_validation->run()) {
            if (!empty($_POST['val'])) {
                if ('sync_quantity' == $this->input->post('form_action')) {
                    ...
                    redirect($_SERVER['HTTP_REFERER']);
                } elseif ('labels' == $this->input->post('form_action')) {
                    foreach ($_POST['val'] as $id) {
    ...
    public function set_rack($product_id = null, $warehouse_id = null)
    {
        ...
        if (true == $this->form_validation->run() && $this->products_model->setRack($data)) {
    ...
    public function update_price()
    {
        ...
        if (true == $this->form_validation->run() && !empty($final)) {
