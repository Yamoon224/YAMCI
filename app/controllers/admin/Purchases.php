...
class Purchases extends MY_Controller
{
    ...
    public function index($warehouse_id = null)
    {
        ...
        // Stats KPI Achats
        $stats = new stdClass();
        $stats->total_purchases = 0; $stats->total_amount = 0; $stats->pending = 0; $stats->credit = 0;
        try {
            ...
            $sql = "SELECT
                ...
                FROM " . $this->db->dbprefix('purchases') . " $whClause";
            $q = $this->db->query($sql);
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
                $stats->total_purchases = (int)   $r->total_purchases;
        ...
        $this->data['suppliers'] = $this->site->getAllCompanies('supplier');
        $this->data['statuses_purchase'] = ['received' => lang('received'), 'pending' => lang('pending'), 'partial' => lang('partial'), 'ordered' => lang('ordered'), 'returned' => lang('returned')];
