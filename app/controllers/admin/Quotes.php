...
class Quotes extends MY_Controller
{
    ...
    public function index($warehouse_id = null)
    {
        ...
        // Stats KPI Devis
        $stats = new stdClass();
        $stats->total_quotes = 0; $stats->total_amount = 0; $stats->pending = 0; $stats->completed = 0;
        try {
            ...
            $sql = "SELECT
                ...
                FROM " . $this->db->dbprefix('quotes') . " $whClause";
            $q = $this->db->query($sql);
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
                $stats->total_quotes = (int)   $r->total_quotes;
                $stats->total_amount = (float) $r->total_amount;
                $stats->pending      = (int)   $r->pending;
                $stats->completed    = (int)   $r->completed;
            }
        } catch (\Throwable $e) { /* silencieux */ }
        $this->data['stats']    = $stats;
        $this->data['billers']  = $this->site->getAllCompanies('biller');
        $this->data['customers'] = $this->site->getAllCompanies('customer');
        $this->data['statuses'] = ['completed' => lang('completed'), 'pending' => lang('pending'), 'sent' => lang('sent')];
