...
class Returns extends MY_Controller
{
    ...
    public function index($warehouse_id = null)
    {
        ...
        // Stats KPI Retours
        $stats = new stdClass();
        $stats->total = 0; $stats->total_amount = 0; $stats->month_count = 0; $stats->month_amount = 0;
        try {
            $whClause = $warehouse_id ? "WHERE warehouse_id = " . (int) $warehouse_id : '';
            $sql = "SELECT
                COUNT(*) AS total,
                COALESCE(SUM(grand_total), 0) AS total_amount,
                SUM(CASE WHEN DATE_FORMAT(date,'%Y-%m') = DATE_FORMAT(NOW(),'%Y-%m') THEN 1 ELSE 0 END) AS month_count,
                COALESCE(SUM(CASE WHEN DATE_FORMAT(date,'%Y-%m') = DATE_FORMAT(NOW(),'%Y-%m') THEN grand_total ELSE 0 END), 0) AS month_amount
                FROM " . $this->db->dbprefix('return_sales') . " $whClause";
            $q = $this->db->query($sql);
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
                $stats->total        = (int)   $r->total;
                $stats->total_amount = (float) $r->total_amount;
                $stats->month_count  = (int)   $r->month_count;
                $stats->month_amount = (float) $r->month_amount;
            }
        } catch (\Throwable $e) { /* silencieux */ }
        $this->data['stats'] = $stats;
        $this->data['customers_list'] = $this->site->getAllCompanies('customer');
