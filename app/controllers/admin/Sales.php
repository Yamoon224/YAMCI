...
class Sales extends MY_Controller
{
    ...
    public function index($warehouse_id = null)
    {
        ...
        /* ────────────────────────────────────────────────────────────────
         * Stats widget (style Pixinvent : 4 KPIs)
         *   - Total ventes  (count)
         *   - CA total      (somme grand_total)
         *   - En attente    (count payment_status = pending)
         *   - Crédit        (somme grand_total - paid des partiellement payés)
         * ──────────────────────────────────────────────────────────────── */
        $stats = new stdClass();
        $stats->total_sales = 0; $stats->total_amount = 0; $stats->pending = 0; $stats->credit = 0;
        try {
            $whClause = $warehouse_id ? "WHERE warehouse_id = " . (int) $warehouse_id : '';
            $sql = "SELECT
                COUNT(*)                                                       AS total_sales,
                COALESCE(SUM(grand_total), 0)                                  AS total_amount,
                SUM(CASE WHEN payment_status = 'pending' THEN 1 ELSE 0 END)    AS pending,
                COALESCE(SUM(GREATEST(grand_total - COALESCE(paid,0), 0)), 0)  AS credit
                FROM " . $this->db->dbprefix('sales') . " $whClause";
            $q = $this->db->query($sql);
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
        ...
        $this->data['billers']  = $this->site->getAllCompanies('biller');
        $this->data['statuses_sale']    = ['completed' => lang('completed'), 'pending' => lang('pending'), 'partial' => lang('partial'), 'returned' => lang('returned')];
        $this->data['statuses_payment'] = ['paid' => lang('paid'), 'pending' => lang('pending'), 'partial' => lang('partial'), 'due' => lang('due')];
