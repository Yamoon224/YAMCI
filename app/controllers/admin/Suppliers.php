...
class Suppliers extends MY_Controller
{
    ...
    public function index($action = null)
    {
        ...
        // Stats KPI Fournisseurs
        $stats = new stdClass();
        $stats->total = 0; $stats->purchases_count = 0; $stats->total_purchases = 0; $stats->credit = 0;
        try {
            $q = $this->db->query("SELECT COUNT(*) AS total FROM " . $this->db->dbprefix('companies') . " WHERE group_name = 'supplier'");
            if ($q && $q->num_rows() > 0) { $stats->total = (int) $q->row()->total; }
            $q = $this->db->query("SELECT COUNT(*) AS nb, COALESCE(SUM(grand_total),0) AS total, COALESCE(SUM(GREATEST(grand_total - COALESCE(paid,0), 0)),0) AS credit FROM " . $this->db->dbprefix('purchases'));
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
                $stats->purchases_count = (int)   $r->nb;
                $stats->total_purchases = (float) $r->total;
                $stats->credit          = (float) $r->credit;
            }
        } catch (\Throwable $e) { /* silencieux */ }
        $this->data['stats'] = $stats;

        // Pays / villes pour selects de filtre
        try {
            $q = $this->db->query("SELECT DISTINCT country FROM " . $this->db->dbprefix('companies') . " WHERE group_name='supplier' AND country IS NOT NULL AND country != '' ORDER BY country");
            $this->data['filter_countries'] = $q ? $q->result() : [];
        } catch (\Throwable $e) { $this->data['filter_countries'] = []; }
        try {
            $q = $this->db->query("SELECT DISTINCT city FROM " . $this->db->dbprefix('companies') . " WHERE group_name='supplier' AND city IS NOT NULL AND city != '' ORDER BY city");
            $this->data['filter_cities'] = $q ? $q->result() : [];
        } catch (\Throwable $e) { $this->data['filter_cities'] = []; }
