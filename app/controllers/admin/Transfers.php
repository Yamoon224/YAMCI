...
class Transfers extends MY_Controller
{
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            try {
                if ($this->transfers_model->updateTransfer($id, $data, $products, $attachments)) {
                    $this->session->set_userdata('remove_tols', 1);
    ...
    public function index()
    {
        ...
        // Stats KPI Transferts
        $stats = new stdClass();
        $stats->total = 0; $stats->completed = 0; $stats->pending = 0; $stats->total_amount = 0;
        try {
            $sql = "SELECT
                COUNT(*) AS total,
                ...
                FROM " . $this->db->dbprefix('transfers');
            $q = $this->db->query($sql);
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
        ...
        // Entrepôts pour selects
        $this->data['warehouses_list'] = $this->site->getAllWarehouses();
