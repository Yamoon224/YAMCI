...
class Welcome extends MY_Controller
{
    ...
    public function index()
    {
        ...
        /* ────────────────────────────────────────────────────────────────
         * User info + stats du mois courant pour le hero card
         * ──────────────────────────────────────────────────────────────── */
        $current_user = $this->site->getUser();
        $this->data['current_user'] = $current_user;

        // Stats mois courant
        $heroStats = new stdClass();
        $heroStats->month_sales       = 0;
        $heroStats->month_sales_count = 0;
        $heroStats->today_sales       = 0;
        $heroStats->today_sales_count = 0;
        $heroStats->customers_count   = 0;
        $heroStats->profit            = 0;
        $heroStats->transactions      = 0;
        try {
            $start_month = date('Y-m-01 00:00:00');
            $end_month   = date('Y-m-d 23:59:59');
            $today       = date('Y-m-d');
            // Ventes du mois en cours
            $q = $this->db->query("SELECT COUNT(*) AS nb, COALESCE(SUM(grand_total),0) AS total
                FROM " . $this->db->dbprefix('sales') . "
                WHERE date BETWEEN ? AND ?", [$start_month, $end_month]);
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
                $heroStats->month_sales       = (float) $r->total;
                $heroStats->month_sales_count = (int)   $r->nb;
            }
            // Ventes du jour
            $q = $this->db->query("SELECT COUNT(*) AS nb, COALESCE(SUM(grand_total),0) AS total
                FROM " . $this->db->dbprefix('sales') . "
            ...
            if ($q && $q->num_rows() > 0) {
                $r = $q->row();
                $heroStats->today_sales       = (float) $r->total;
                $heroStats->today_sales_count = (int)   $r->nb;
            }
            // Clients
            $heroStats->customers_count = (int) $this->db->where('group_name', 'customer')->count_all_results('companies');
            // Transactions
            $heroStats->transactions = (int) $this->db->count_all_results('payments');
        } catch (\Throwable $e) { /* silencieux */ }
        $this->data['heroStats'] = $heroStats;
