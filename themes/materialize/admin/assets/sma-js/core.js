...
function cssStyle() {
    ...
    if ($('#sidebar-left').hasClass('minified')) {
        ...
        if ($.cookie('sma_theme_fixed') == 'yes') {
            ...
            if ($.fn.perfectScrollbar) { $('#sidebar-left').perfectScrollbar({ suppressScrollX: true }); }
        } else {
            $('#content, #sidebar-left, #header').removeAttr('style');
            $('#sidebar-left').removeClass('sidebar-fixed');
            $('#content').removeClass('content-with-fixed');
            ...
            $('#main-menu-act').addClass('full visible-md visible-lg').show();
            $('#fixed').removeClass('fixed');
            if ($.fn.perfectScrollbar) { $('#sidebar-left').perfectScrollbar('destroy'); }
        }
    }
    widthFunctions();
}
...
$(document).ready(function () {
    if ($.fn.autocomplete) { $('#suggest_product').autocomplete({
    ...
    if ($.fn.autocomplete) { $('#suggest_product2').autocomplete({
        source: site.base_url + 'reports/suggestions',
        select: function (event, ui) {
        ...
        response: function (event, ui) {
            if (ui.content.length == 1 && ui.content[0].id != 0) {
                ui.item = ui.content[0];
                $(this).val(ui.item.label);
                if ($(this).data('ui-autocomplete')) { $(this).data('ui-autocomplete')._trigger('select', 'autocompleteselect', ui); }
                $(this).autocomplete('close');
                $(this).removeClass('ui-autocomplete-loading');
            }
        },
    ...
    if ($.fn.perfectScrollbar) { $('.top-menu-scroll').perfectScrollbar(); }
...
$(document).ready(function () {
    ...
    // Redactor (Imperavi) is NOT loaded in the Materialize theme.
    // Only initialize if the plugin is available.
    if ($.fn.redactor) {
        $('textarea')
            .not('.skip')
            .redactor({
...
$(document).ready(function () {
    ...
    if ($.fn.calculator) { $('#inlineCalc').calculator({ layout: ['_%+-CABS', '_7_8_9_/', '_4_5_6_*', '_1_2_3_-', '_0_._=_+'], showFormula: true }); }
    ...
    $('#daterange').daterangepicker(
        {
            ...
            ranges: {
                Aujourdhui: [moment().hours(0).minutes(0).seconds(0), moment()],
                Hier: [
                    moment().subtract('days', 1).hours(0).minutes(0).seconds(0),
                    moment().subtract('days', 1).hours(23).minutes(59).seconds(59),
                ],
                '7 derniers jours': [moment().subtract('days', 6).hours(0).minutes(0).seconds(0), moment().hours(23).minutes(59).seconds(59)],
                '30 derniers jours': [moment().subtract('days', 29).hours(0).minutes(0).seconds(0), moment().hours(23).minutes(59).seconds(59)],
                'Ce mois': [
...
function pqFormat(x) {
    if (x != null) {
        ...
        for (index = 0; index < pqc.length; ++index) {
            var pq = pqc[index];
            var v = pq.split('__');
            d += v[0] + (v[2] ? ' (' + v[2] + ': ' : ' (') + '' + formatQuantity2(v[1]) + ')<br>';
        }
        return d;
    } else {
        return '';
...
$(document).ready(function () {
    if (typeof site !== 'undefined' && site.settings && site.settings.auto_detect_barcode == 1) {
        var pressed = false;
        var chars = [];
                ...
                if (pressed == false) {
                    setTimeout(function () {
                        if (chars.length >= 8) {
                            ...
                            if ($.fn.autocomplete) { $('#add_item').focus().autocomplete('search', barcode); }
                        }
                        chars = [];
                        pressed = false;
                    }, 200);
...
if ($.fn.sortable) { $('.sortable_table tbody').sortable({ containerSelector: 'tr' }); }
