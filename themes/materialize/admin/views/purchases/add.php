...
<script type="text/javascript">
    ...
    var poKeys = ['poitems','podiscount','potax2','poshipping','poref','powarehouse',
                  'ponote','posupplier','pocurrency','poextras','podate','postatus','popayment_term'];
    ...
    <?php if ($quote->supplier_id) { ?>localStorage.setItem('posupplier', '<?= $quote->supplier_id ?>');<?php } ?>
    localStorage.setItem('poitems', JSON.stringify(<?= $quote_items ?>));
    <?php } ?>

    var count = 1, an = 1, po_edit = false, product_variant = 0,
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '#addItemManually', function () {
            var fields = {code:'mcode',name:'mname',category:'mcategory',unit:'munit',cost:'mcost',price:'mprice'};
            var msgs = {
...
<?php
$attrib = ['data-toggle' => 'validator', 'role' => 'form', 'id' => 'purchasesAddForm'];
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-6 row-gap-4">
  <div class="d-flex flex-column justify-content-center">
    <h4 class="mb-1"><?= lang('add_purchase') ?: 'Nouvel achat' ?></h4>
    <p class="mb-0 text-muted">Créez un nouvel achat fournisseur et ses lignes de produits</p>
    <nav aria-label="breadcrumb" class="mt-2">
      <ol class="breadcrumb breadcrumb-style1 mb-0">
        ...
        <li class="breadcrumb-item active" aria-current="page"><?= lang('add_purchase') ?></li>
      </ol>
    </nav>
  </div>
  <div class="d-flex align-content-center flex-wrap gap-2">
    ...
    <button type="button" class="btn btn-primary" onclick="$('#purchasesAddForm').submit();">
...
<div class="card mb-4">
    <div class="card-header">
        <h5 class="card-title mb-0"><span class="ri-file-list-line me-2"></span><?= lang('purchase_details') ?></h5>
    </div>
    <div class="card-body">
        <div class="row g-3">
            <?php if ($Owner || $Admin) { ?>
            <div class="col-md-4">
                ...
                <input type="text" name="date" value="<?= $_POST['date'] ?? '' ?>"
                       class="form-control datetime" id="podate" required />
            </div>
            <?php } ?>

            <div class="col-md-4">
                <label for="poref" class="form-label"><?= lang('reference_no') ?></label>
                <?= form_input('reference_no', ($_POST['reference_no'] ?? $ponumber), 'class="form-control" id="poref"') ?>
            </div>
            ...
            <div class="col-md-4">
                ...
                echo form_dropdown('status', $post, ($_POST['status'] ?? ''), 'id="postatus" class="form-select" required');
                ?>
            </div>
...
<div class="card mb-4 border-warning">
    <div class="card-header bg-warning bg-opacity-10">
        <h6 class="card-title mb-0 text-warning">
            <span class="ri-alert-line me-1"></span><?= lang('please_select_these_before_adding_product') ?>
        </h6>
    </div>
    <div class="card-body">
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <a href="#" id="view-supplier" class="btn btn-outline-secondary external no-print"
                    ...
                    <a href="<?= admin_url('suppliers/add') ?>" id="add-supplier"
                       class="btn btn-outline-primary external no-print"
                       data-bs-toggle="modal" data-bs-target="#myModal" title="<?= lang('add_supplier') ?>">
...
<div class="card mb-4" id="sticker">
    <div class="card-body">
        <div class="input-group input-group-lg">
            <span class="input-group-text"><span class="ri-barcode-line ri-lg"></span></span>
            <?= form_input('add_item', '', 'class="form-control" id="add_item" placeholder="' . lang('add_product_to_order') . '"') ?>
            <?php if ($Owner || $Admin || $GP['products-add']) { ?>
            <a href="<?= admin_url('products/add') ?>" id="addManually1" class="btn btn-outline-secondary" title="<?= lang('add_product') ?>">
                <span class="ri-add-circle-line ri-lg"></span>
            </a>
            <?php } ?>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table id="poTable" class="table items table-striped table-bordered table-hover mb-0 sortable_table">
                ...
                <tr>
                    ...
                    <th style="width:40px; text-align:center;"><span class="ri-delete-bin-line text-danger"></span></th>
                </tr>
                </thead>
                <tbody></tbody>
                <tfoot></tfoot>
...
<div class="d-flex gap-2 justify-content-end mb-4">
    <button type="button" class="btn btn-secondary" id="reset">
        <span class="ri-refresh-line me-1"></span><?= lang('reset') ?>
    </button>
    <?= form_submit('add_pruchase', lang('submit'), 'id="add_pruchase" class="btn btn-primary"') ?>
</div>
                            ...
                            <div class="card-body p-2">
                                ...
                                <div class="input-group input-group-sm">
                                    ...
                                    <a href="#" id="calculate_unit_price" class="btn btn-outline-secondary" title="<?= lang('calculate_unit_cost') ?>">
                                        <span class="ri-calculator-line"></span>
                                    </a>
                                </div>
                            </div>
                        </div>
