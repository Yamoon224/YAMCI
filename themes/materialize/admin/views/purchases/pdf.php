...
<head>
    ...
    <?php if ($Settings->user_rtl) {
     ...
     <style>
        body {
            font-size: 12px; /* Modifier la taille du texte globalement */
        }
        .table td, .table th {
            font-size: 10px; /* Modifier la taille du texte dans le tableau */
        }
    </style>
</head>
...
<div id="wrap">
    <div class="row">
        <div class="col-lg-12">
           ...
           <?php
if ($logo) {
    // Chemin fixe du logo, nom du fichier : logoo.png
    $path   = base_url() . 'assets/uploads/logos/logoo.png';
    $type   = pathinfo($path, PATHINFO_EXTENSION);
    $data   = file_get_contents($path);
    $base64 = 'data:image/' . $type . ';base64,' . base64_encode($data); ?>
    <div class="text-center" style="margin-bottom:20px;">
        <img src="<?= $base64; ?>" alt="<?=$Settings->site_name; ?>" width="300" height="80">
    </div>
<?php
}
?>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped">
                    ...
                    foreach ($rows as $row) :
                        ?>
                            ...
                            <?php if ($Settings->indian_gst) {
                                ...
                                <?php
                                if ($Settings->tax1 && $inv->product_tax > 0) {
                                    echo '<td class="text-right" style="width: 100px; vertical-align:middle;">' . ($row->item_tax != 0 ? '<small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' : '') . $this->sma->formatMoney($row->item_tax) . '</td>';
                                }
                                ?>
                                <?php
                                if ($Settings->product_discount && $inv->product_discount != 0) {
                                    echo '<td class="text-right" style="width: 100px; vertical-align:middle;">' . ($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->sma->formatMoney($row->item_discount) . '</td>';
                                }
                                ?>
                                <td class="text-right" style="width:120px;"><?=$this->sma->formatMoney($row->subtotal);?></td>
                            </tr>
                            <?php
                            $r++;
                    endforeach;
                        ...
                        foreach ($return_rows as $row) :
                            ?>
                                ...
                                if ($inv->status == 'partial') {
                                    ...
                                    <td class="text-right" style="width:100px;"><?=$this->sma->formatMoney($row->unit_cost); ?></td>
                                    <?php
                                    if ($Settings->tax1 && $inv->product_tax > 0) {
                                        echo '<td class="text-right" style="width: 100px; vertical-align:middle;">' . ($row->item_tax != 0 ? '<small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' : '') . $this->sma->formatMoney($row->item_tax) . '</td>';
                                    }
                    ...
                    <?php if ($inv->grand_total != $inv->total) {
                        ?>
                        <tr>
                            <td class="text-right" colspan="<?= $tcol; ?>"
                               ><?= lang('total'); ?>
                    ...
                    <?php
                    if ($return_purchase) {
                        echo '<tr><td class="text-right" colspan="' . $col . '">' . lang('return_total') . ' (' . $default_currency->code . ')</td><td class="text-right">' . $this->sma->formatMoney($return_purchase->grand_total) . '</td></tr>';
                    }
                    if ($inv->surcharge != 0) {
                    ...
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr><td class="text-right" colspan="' . $col . '">' . lang('order_discount') . ' (' . $default_currency->code . ')</td><td class="text-right">' . ($inv->order_discount_id ? '<small>(' . $inv->order_discount_id . ')</small> ' : '') . $this->sma->formatMoney($return_purchase ? ($inv->order_discount + $return_purchase->order_discount) : $inv->order_discount) . '</td></tr>';
                    }
                    ?>
                    <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                        echo '<tr><td class="text-right" colspan="' . $col . '">' . lang('order_tax') . ' (' . $default_currency->code . ')</td><td class="text-right">' . $this->sma->formatMoney($return_purchase ? ($inv->order_tax + $return_purchase->order_tax) : $inv->order_tax) . '</td></tr>';
                    }
                    ?>
                    <?php if ($inv->shipping != 0) {
                        echo '<tr><td class="text-right" colspan="' . $col . '">' . lang('shipping') . ' (' . $default_currency->code . ')</td><td class="text-right">' . $this->sma->formatMoney($inv->shipping) . '</td></tr>';
                    }
                    ?>
                    <tr>
                        <td class="text-right" colspan="<?= $col; ?>"
                            style="font-weight:bold;"><?= lang('total_amount'); ?>
                            (<?= $default_currency->code; ?>)
                        </td>
                        <td class="text-right" style="font-weight:bold;"><?= $this->sma->formatMoney($return_purchase ? ($inv->grand_total + $return_purchase->grand_total) : $inv->grand_total); ?></td>
                    </tr>
                    <tr>
                        <td class="text-right" colspan="<?= $col; ?>"
                            style="font-weight:bold;"><?= lang('paid'); ?>
                            (<?= $default_currency->code; ?>)
                        </td>
                        <td class="text-right" style="font-weight:bold;"><?= $this->sma->formatMoney($return_purchase ? ($inv->paid + $return_purchase->paid) : $inv->paid); ?></td>
                    </tr>
                    <tr>
                        <td class="text-right" colspan="<?= $col; ?>"
                            style="font-weight:bold;"><?= lang('Reste à payer'); ?>
                            (<?= $default_currency->code; ?>)
                        </td>
                        <td class="text-right" style="font-weight:bold;"><?= $this->sma->formatMoney(($return_purchase ? ($inv->grand_total + $return_purchase->grand_total) : $inv->grand_total) - ($return_purchase ? ($inv->paid + $return_purchase->paid) : $inv->paid)); ?></td>
                    </tr>
