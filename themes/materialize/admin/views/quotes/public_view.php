...
<head>
    ...
    <title><?= $this->lang->line('quote') . ' ' . $inv->reference_no; ?></title>
    ...
    <style>
        ...
        .invoice-header { border-bottom: 2px solid #71dd37; padding-bottom: 20px; margin-bottom: 24px; }
        .company-logo img { max-height: 70px; }
        .invoice-title { font-size: 2rem; font-weight: 700; color: #71dd37; }
        .info-label { font-size: 0.75rem; text-transform: uppercase; color: #888; letter-spacing: .05em; }
        .info-value { font-weight: 600; }
        .table thead th { background: #71dd37; color: #fff; border: none; }
        .table tfoot td, .table tfoot th { font-weight: 600; }
        .total-row td { font-size: 1.05rem; }
        .grand-total td { font-size: 1.2rem; font-weight: 700; color: #71dd37; border-top: 2px solid #71dd37 !important; }
        .note-box { background: #f8f9fa; border-left: 4px solid #71dd37; padding: 12px 16px; border-radius: 4px; }
        .quote-badge { background: #e8f9ee; color: #28a745; font-size: 0.75rem; padding: 3px 10px; border-radius: 20px; font-weight: 600; text-transform: uppercase; }
        @media print {
            body { background: #fff; }
...
<div class="invoice-wrap">
    ...
    <div class="invoice-header d-flex justify-content-between align-items-start flex-wrap gap-3">
        ...
        <div class="text-end">
            <div class="invoice-title mb-1"><?= lang('quote'); ?></div>
            <span class="quote-badge"><?= lang('quotation'); ?></span>
            <div class="text-muted mt-1"><?= htmlspecialchars($Settings->site_name); ?></div>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-sm-5">
            ...
            <?php foreach (['cf1'=>'ccf1','cf2'=>'ccf2','cf3'=>'ccf3','cf4'=>'ccf4','cf5'=>'ccf5','cf6'=>'scf6'] as $cf => $lk):
    ...
    <div class="row mb-4">
        <div class="col-sm-5">
            ...
            <div class="info-value"><?= htmlspecialchars($Settings->site_name); ?></div>
            <div class="text-muted"><?= htmlspecialchars($warehouse->name); ?></div>
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <?php if ($Settings->product_discount) echo '<th class="text-end">' . lang('discount') . '</th>'; ?>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php if ($Settings->product_discount) echo '<td class="text-end">' . ($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->sma->formatMoney($row->item_discount) . '</td>'; ?>
            ...
            <tfoot>
                <tr>
                    ...
                    <?php if ($Settings->tax1 && $inv->product_tax > 0) echo '<td class="text-end">' . $this->sma->formatMoney($inv->product_tax) . '</td>'; ?>
                    <?php if ($Settings->product_discount) echo '<td class="text-end">' . $this->sma->formatMoney($inv->product_discount) . '</td>'; ?>
                    <td class="text-end"><?= $this->sma->formatMoney($inv->total + $inv->product_tax); ?></td>
                </tr>
                <?php if ($Settings->indian_gst):
                    if ($inv->cgst > 0) echo '<tr><td colspan="' . $col . '" class="text-end">' . lang('cgst') . ' (' . $default_currency->code . ')</td><td class="text-end">' . ($Settings->format_gst ? $this->sma->formatMoney($inv->cgst) : $inv->cgst) . '</td></tr>';
                    if ($inv->sgst > 0) echo '<tr><td colspan="' . $col . '" class="text-end">' . lang('sgst') . ' (' . $default_currency->code . ')</td><td class="text-end">' . ($Settings->format_gst ? $this->sma->formatMoney($inv->sgst) : $inv->sgst) . '</td></tr>';
                    if ($inv->igst > 0) echo '<tr><td colspan="' . $col . '" class="text-end">' . lang('igst') . ' (' . $default_currency->code . ')</td><td class="text-end">' . ($Settings->format_gst ? $this->sma->formatMoney($inv->igst) : $inv->igst) . '</td></tr>';
                endif; ?>
                <?php if ($inv->order_discount != 0) echo '<tr><td colspan="' . $col . '" class="text-end">' . lang('order_discount') . ' (' . $default_currency->code . ')</td><td class="text-end">' . ($inv->order_discount_id ? '<small>(' . $inv->order_discount_id . ')</small> ' : '') . $this->sma->formatMoney($inv->order_discount) . '</td></tr>'; ?>
                <?php if ($Settings->tax2 && $inv->order_tax != 0) echo '<tr><td colspan="' . $col . '" class="text-end">' . lang('order_tax') . ' (' . $default_currency->code . ')</td><td class="text-end">' . $this->sma->formatMoney($inv->order_tax) . '</td></tr>'; ?>
                <?php if ($inv->shipping != 0) echo '<tr><td colspan="' . $col . '" class="text-end">' . lang('shipping') . ' (' . $default_currency->code . ')</td><td class="text-end">' . $this->sma->formatMoney($inv->shipping) . '</td></tr>'; ?>
