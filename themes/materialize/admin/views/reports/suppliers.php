...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-6 row-gap-4">
  <div class="d-flex flex-column justify-content-center">
    <h4 class="mb-1">
        <i class="ri ri-truck-line me-2" style="font-size:24px;vertical-align:-0.2em"></i>
        <?php echo lang('suppliers') ?: 'Rapport fournisseurs'; ?>
      </h4>
    <nav aria-label="breadcrumb" class="mt-2">
      <ol class="breadcrumb breadcrumb-style1 mb-0">
        ...
        <li class="breadcrumb-item active"><?php echo lang('suppliers') ?: 'Fournisseurs'; ?></li>
      </ol>
    </nav>
    </div>
...
<!-- Filter Card (column-level search is handled in DataTable footer, but we also expose a simple top filter) -->
    ...
    <div class="row g-4">
      <div class="col-sm-6 col-md-4">
        <div class="form-floating form-floating-outline">
          <input type="text" class="form-control" id="filterSupplierName" placeholder="Nom fournisseur" />
          <label for="filterSupplierName"><?php echo lang('name') ?: 'Nom'; ?></label>
        </div>
      </div>
      <div class="col-sm-6 col-md-4">
        <div class="form-floating form-floating-outline">
          <input type="text" class="form-control flatpickr-date" id="filterStartDate" placeholder="Date début" />
          <label for="filterStartDate"><?php echo lang('start_date') ?: 'Date début'; ?></label>
        </div>
      </div>
      <div class="col-sm-6 col-md-4">
        <div class="form-floating form-floating-outline">
          <input type="text" class="form-control flatpickr-date" id="filterEndDate" placeholder="Date fin" />
          <label for="filterEndDate"><?php echo lang('end_date') ?: 'Date fin'; ?></label>
        </div>
      </div>
    </div>
  </div>
    ...
    <table id="CusData" class="table datatables-ajax border-top">
      <thead>
        <tr>
          ...
          <th><?php echo lang('total_purchases') ?: 'Total achats'; ?></th>
      ...
      <tfoot class="dtFilter">
        <tr>
          ...
          <th class="text-center"><?php echo lang('total_purchases') ?: 'Achats'; ?></th>
          <th class="text-center"><?php echo lang('total_amount') ?: 'Montant'; ?></th>
...
(function () {
  ...
  var oTable = $('#CusData').DataTable({
    ...
    order: [[1, 'asc']],
    ajax: {
      url: '<?php echo admin_url('reports/getSuppliers'); ?>',
      type: 'POST',
      data: function (d) {
    ...
    columns: [
      null, null, null, null,
      { searchable: false, render: function (d) { return decimalFormat ? decimalFormat(parseFloat(d)) : d; } },
      ...
      { searchable: false, render: function (d) {
          ...
          return val > 0
            ? '<span class="text-danger fw-semibold">' + currencyFormat(val) + '</span>'
            : '<span class="text-success fw-semibold">' + currencyFormat(val) + '</span>';
        }
      },
      { orderable: false }
    ],
    footerCallback: function (row, data, start, end, display) {
      ...
      cells[4].innerHTML = decimalFormat ? decimalFormat(purchases) : purchases;
      ...
      cells[7].innerHTML = balance > 0
        ...
        : '<span class="text-success">' + currencyFormat(balance) + '</span>';
    },
    lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, '<?php echo lang('all') ?: 'Tout'; ?>']],
    dom: '<"row"<"col-sm-12 col-md-6"l><"col-sm-12 col-md-6"f>>t<"row"<"col-sm-12 col-md-5"i><"col-sm-12 col-md-7"p>>'
  });

  // Column search from filter
  $('#filterSupplierName').on('keyup change', function () {
    oTable.column(1).search(this.value).draw();
  });

  document.getElementById('xls').addEventListener('click', function (e) {
    e.preventDefault();
    window.location.href = '<?php echo admin_url('reports/getSuppliers/0/xls'); ?>';
  });
})();
</script>
